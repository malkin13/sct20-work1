...
Person[] generatePersons(int elements) {
    ...
    Person[] array = new Person[elements];
    ...
    for (int i=0;i<elements;i++) {
        Object randomName = namesWithSexMap.keySet().toArray()[new Random().nextInt(namesWithSexMap.keySet().toArray().length)];
